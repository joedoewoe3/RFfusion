 lib_dir = Base.joinpath(Base.dirname(@__DIR__), "lib")
 lib_ext = Sys.iswindows() ? "dll" : Sys.isapple() ? "dylib" : "so"
 lib_path = Base.joinpath(lib_dir, "libRFWrapper.$lib_ext")
 const LIB_RF = Base.joinpath(lib_dir, lib_path)  # Adapt ext as above
 dlopen(LIB_RF)  # Load at module init
ENV["GKSwtype"] = "null"

const ROT_ACTION = RotationAction(Euclidean(3), SpecialOrthogonal(3), LeftAction())  # Left for standard rot

function quat_to_rot(q::Vector{Float64})
    q = q / norm(q)  # Normalize if not unit (safety, though MEKFs keep it unit)
    w, x, y, z = q
    R = [1 - 2y^2 - 2z^2   2x*y - 2z*w   2x*z + 2y*w;
...
function quat_mult(q1::Vector{Float64}, q2::Vector{Float64})
    ...
    q = [w, x, y, z]
    return q / norm(q)  # Normalize for unit quat
end

# If δR is matrix (3x3 or 4x4), convert to quat first (add helper)
function rot_matrix_to_quat(R::Matrix{Float64})
    # Assume 3x3; if 4x4 homogeneous, slice R[1:3,1:3]
    if size(R) == (4,4)
        R = R[1:3,1:3]
    end
    tr = LinearAlgebra.tr(R)
    if tr > 0
        s = sqrt(tr + 1) * 2
        w = s / 4
        x = (R[3,2] - R[2,3]) / s
    ...
    q = [w, x, y, z]
    return q / norm(q)  # Unit quat
end


function quat_rotate(q::Vector{Float64}, v::Vector{Float64})
    q = q / norm(q)  # Safety norm
    v_pure = [0.0; v[1:3]]  # Pure quat from 3D vec (drop extra if dim=4 pad)
    q_inv = [q[1]; -q[2:4]] / norm(q)  # Conj for unit quat
    h_pure = quat_mult(quat_mult(q, v_pure), q_inv)  # Sandwich: q * v * q^{-1}
    return [h_pure[2:4]; v[4]]  # Back to 4D vec (rotated 3D + original pad)
end
...
function generate_primes(n::Int)
    ...
    function e8_root_lattice()
        ...
        function sensor_fusion_ring_mekf(ω::Vector{Float64}, dt::Float64, state_prev::Union{Vector{Float64}, OctonionF64}, v_measured::Vector{Float64}, v_predicted::Vector{Float64}, P_Previous::AbstractMatrix{Float64}, Q::AbstractMatrix{Float64}, R::AbstractMatrix{Float64}, dim::Int=3, mode::String="manifolds")
            if mode == "manifolds"
                ...
                ΔR = exp(SO_dim, e, Ω)
                if sum(state_prev) == 0 && state_prev[1] == 0; state_prev[1] = 1.0; end
                if length(state_prev) == 4; R_gyro = quat_to_rot(state_prev); end
                F = Matrix(I, dim, dim) + Ω
                P_Predicted = F * P_Previous * F' + Q
                h = R_gyro * v_predicted[1:3]  # Slice to 3D if needed; extend h with v[4] if scalar
                #h = apply(ROT_ACTION, R_gyro, v_predicted[1:3])  # Slice to 3D if v_pred is 4D; adjust if full 4D
                # if isnothing(v_measured) v_measured = [0.0, 0.0, -9.80665, zeros(dim-3)...]; end
                # if isnothing(v_predicted); v_predicted = [0.0, 0.0, -9.80665, zeros(dim-3)...]; end;
                R_so = Manifolds.embed(SpecialOrthogonal(3), R_gyro)
                v_3d = v_predicted[1:3]  # Slice to 3D gravity vec (drop trailing 0)
                h_3d = apply(ROT_ACTION, R_so, v_3d)  # Or just R_gyro * v_3d for simple matrix mult
                h = [h_3d; v_predicted[4]]
                ...
                δq = rot_matrix_to_quat(δR)  # Convert δR (4x4 or 3x3) to quat vector
                R_gyro_quat = rot_matrix_to_quat(R_gyro)  # If R_gyro not already quat; skip if state_prev is quat
                state_fused = quat_mult(δq, R_gyro_quat)  # Multiplicative update: δq ⊗ q_gyro
               ...
               # @assert dim == 7 "Octo mode fixed to 7D imaginaries"
                ...
                @assert length(v_measured) == dim && length(v_predicted) == dim "Vectors R^8 for E8 embed"
                    ...
                    if abs(o_prev) ≈ 0; o_prev = Octonion(1.0); end
                    ω_oct = pad_to_oct(ω) * dt / 2.0
                    Δo = (Octonion(1.0) + ω_oct) / abs(Octonion(1.0) + ω_oct)
                    o_gyro = Δo * o_prev
                    o_gyro /= abs(o_gyro)
            ...
            function simulate_fusion(steps::Int=200, dim::Int=8, mode::String="e8"; q_scale=1.0, r_scale=1.0)
                # Init for e8: Unit octo embed
                if mode == "e8"
                    state_prev = Octonion(1.0)
                end
                if mode == "manifolds"
                    state_prev =zeros(Float64, dim)
                    elseif mode == "octonions"
                    state_prev = Octonion(1.0)
                end
                ...
                Q = q_scale * 0.001 * Matrix(I, dim, dim)  # Scaled process noise
                R = r_scale * 0.005 * Matrix(I, dim, dim)
                R[4:end, 4:end] .+= diagm(1e6 * ones(dim-3))  # Keep the extra penalty
                ...
                for i in 1:steps
                    ...
                    if mode == "manifolds"
                      h_fused = quat_rotate(state_new, v_predicted)
                        elseif mode == "octonions"
                        v_pred_oct = pad_to_oct(v_predicted[1:3])
            ...
            function project_to_6d(o_fused::Octonions.OctonionF64)
...
#=
function create_fused_packet(ω, dt, state_prev, v_meas, v_pred, P_prev, Q, R, dim, mode, target_ip, port)
    state_new, P_new, CR = sensor_fusion_ring_mekf(ω, dt, state_prev, v_meas, v_pred, P_prev, Q, R, dim, mode)
    pkt = create_raw_packet(state_new.x, vec(P_new), model_version=1)  # Adjust for your state
    bytes = encode_packet(pkt)
    send_packet_udp(bytes, target_ip, port)
    return state_new, P_new, CR
end
=#

function create_fused_packet(state::Union{Vector{Float64}, OctonionF64}, cov_flat::Vector{Float64}, cr_bounds::Vector{Float64}; model_version=1, stream_id=1, sequence=0, flags=0, label="FusedState", msg="E8/CR Update v$model_version")

    state_vec = state isa OctonionF64 ? to_vec(state) : state

    fused_data = vcat(state_vec, cov_flat, cr_bounds)  # Flat Vector{Float64}
    rf_blob = reinterpret(UInt8, fused_data)  # Bytes pack

    header = make_header(UInt32(stream_id), UInt32(sequence), UInt32(flags))
...
function process_incoming(pkt::RFpacket_pb.RFPacket, src)
